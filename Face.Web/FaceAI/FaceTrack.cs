...
namespace Face.Web.Core.FaceAI
{
    ...
    public class FaceTrack
    {
        ...
        /// 人脸检测，返回人脸信息
        /// </summary>
        /// <param name="file_name">人脸图片信息，传入图片文件路径</param>
        /// <param name="track_num"> 设置最多检测人数（多人脸检测），默认为1，最多可设为10</param>
        /// <returns></returns>
        public string Track(string file_name,int track_num)
        {
            int max_track_num = track_num < 1 ? 1 : track_num;// 设置最多检测人数（多人脸检测），默认为1，最多可设为10
            IntPtr ptr = track(file_name, max_track_num);
            string buf = Marshal.PtrToStringAnsi(ptr);
            Console.WriteLine("track res is:" + buf);
            return buf;
        ...
        /// 测试人脸检测
        /// </summary>
        /// <param name="file_name">图片信息（数据大小应小于10M），文件指定路径</param>
        /// <returns></returns>
        public string TrackMaxFace(string file_name)
        {
            IntPtr ptr = track_max_face(file_name);
            string buf = Marshal.PtrToStringAnsi(ptr);
            Console.WriteLine("track_max_face res is:" + buf);
        ...
        /// 测试人脸检测
        /// </summary>
        /// <param name="file_name">人脸图片信息，二进制图片buffer内容</param>
        /// <param name="track_num">设置最多检测人数（多人脸检测），默认为1，最多可设为10</param>
        /// <returns></returns>
        public string TrackByBuf(byte[] file_name,int track_num)
        {
            // System.Drawing.Image img = System.Drawing.Image.FromFile(file_name);
            byte[] img_bytes = file_name;  // ImageUtil.img2byte(img);
            int max_track_num = track_num < 2 ? 2 : track_num; ; // 设置最多检测人数（多人脸检测），默认为1，最多可设为10
            IntPtr ptr = track_by_buf(img_bytes, img_bytes.Length, max_track_num);
            string buf = Marshal.PtrToStringAnsi(ptr);
            Console.WriteLine("track_by_buf res is:" + buf);
            return buf;
        ...
        /// 检测最大人脸。
        /// </summary>
        /// <param name="file">人脸图片信息，二进制图片buffer内容</param>
        /// <returns></returns>
        public string TrackMaxFaceByBuf(string file)
        {
            System.Drawing.Image img = System.Drawing.Image.FromFile("d:\\2.jpg");
            byte[] img_bytes = ImageUtil.img2byte(img);
            IntPtr ptr = track_max_face_by_buf(img_bytes, img_bytes.Length);
